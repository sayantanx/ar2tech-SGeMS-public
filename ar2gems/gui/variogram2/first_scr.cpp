...
bool First_scr::load_experimental_variograms() {
  ...
  if( !parsed ) {
    GsTLcerr << "File " << String_Op::qstring2string(filename) << " is not a valid XML file\n" << gstlIO::end;
    return false;
  }
...
bool First_scr::extract_direction( GsTLVector<double>& dir,
  ...
  QStringList coords = dir_str.split( " ", QString::SkipEmptyParts );
  if( coords.size() != 3 ) {
...
extract_values( std::vector<double>& values, const QString& tagname,
  ...
  if( node.count() == 0 ) {
    GsTLcerr << "missing tag \"" << String_Op::qstring2string(tagname) << "\" for variogram " 
             << id << gstlIO::end;
    return false;
  }
  QDomElement dir_elem = node.item(0).toElement();
