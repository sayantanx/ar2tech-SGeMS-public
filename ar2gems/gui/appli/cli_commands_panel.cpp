...
void CLI_commands_panel::init() {
  ...
  QWidget* commands_tab = new QWidget( this);
  commands_tab->setObjectName("command_tabs");
  
  QVBoxLayout* layout = new QVBoxLayout( commands_tab );
  layout->setMargin( 7 );
  
  log_window_ = new QTextEdit( commands_tab);
  log_window_->setObjectName("message_box");
  
  log_window_->setWordWrapMode( QTextOption::NoWrap );
  //log_window_->setTextFormat( Qt::LogText );
  log_window_->setReadOnly( true );

  QLabel* label2 = new QLabel( "Run Command:", commands_tab);

  //TL modified
  //command_edit_ = new QLineEdit( commands_tab, "command_edit");
  command_edit_ = new LineEditKey( commands_tab);
  _hs = new HistoryScribe(command_edit_);

  QPushButton* clear_line_button = 
    new QPushButton( "Clear", commands_tab);
  QPushButton* run_script_button = 
    new QPushButton( "Execute Commands File...", commands_tab);
  ...
  QTextEdit* script_output = new QTextEdit( this);
  script_output->setWordWrapMode( QTextOption::NoWrap );
  script_output->setReadOnly( true );

  tab_widget->addTab( commands_tab, "SGeMS Commands History" );
  tab_widget->addTab( script_output, "Scripts Output" );
...
void CLI_commands_panel::run_script() {
  QString filename = 
    QFileDialog::getOpenFileName( this, "script dialog");
  ...
  if( !file.open( QIODevice::ReadOnly ) ) {
    ...
    GsTLcerr << "Can not open " <<  tmp.constData()
  ...
  while ( !stream.atEnd() ) {
    ...
    line = tmp.trimmed().constData();
    if( line.startsWith( "#" ) || line.isEmpty() ) continue;

    Error_messages_handler error_messages;
    tmp = line.toAscii();
    std::string command( tmp.constData() );
