...
<
  ...
  class Grid
>
class SK_local_mean_combiner_impl : public Kriging_combiner_impl<InputIterator,Neighborhood> {
 public:

  // Constructor
  SK_local_mean_combiner_impl(){}

  // Constructor
  SK_local_mean_combiner_impl(std::map<const Grid*,const DataProperty*> lvmeans)
    : lvmeans_(lvmeans){}

  // Clone function
  virtual Kriging_combiner_impl<InputIterator,Neighborhood>* clone() const {
    return new 
      SK_local_mean_combiner_impl<InputIterator,
                             Neighborhood,
                             DataProperty,
                             Grid>( lvmeans_ );
  }

  void add_lvm(const Grid* grid, const DataProperty* lvm) {
    lvmeans_[grid] = lvm;
  }
  ...
  virtual double operator() ( InputIterator begin_weights,
    ...
    typename Neighborhood::const_iterator end = neighbors.end();
    for( ; it != end ; ++it ) {
      typename grid_to_lvmT::const_iterator it_lvm = lvmeans_.find(it->grid());

      int nodeid = it->node_id();
      if( !it_lvm->second->is_informed(nodeid)) {
        return -999;
      }
// This is not efficient.  Should be done apriori on the hard data, not at very point on the grid.
      result += (*begin_weights) * ( it->property_value() - it_lvm->second->get_value_no_check(nodeid) );
      begin_weights++;
    }
    
    typename grid_to_lvmT::const_iterator it_lvm = lvmeans_.find(neighbors.center().grid());
    int nodeid = neighbors.center().node_id();
    if( !it_lvm->second->is_informed(nodeid)) {
      return -999;
    }

    result += it_lvm->second->get_value_no_check(nodeid);
    return result;
  }

 protected:
   //typedef typename std::map<const Grid*, MeanNeighborhood*> grid_to_neighborhoodT;
   typedef typename std::map<const Grid*, const DataProperty*> grid_to_lvmT;
   grid_to_lvmT lvmeans_;
//  mutable MeanNeighborhood mean_neigh_;
//  mutable MeanNeighborhood hd_mean_neigh_;
};
