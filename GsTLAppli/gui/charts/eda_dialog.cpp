...
EDA_dialog::EDA_dialog(QWidget *parent)
  : QMainWindow (parent)
{
...
//  QSplitter* mainSplitter = new QSplitter(Qt::Horizontal, this);
  ...
  if(dir) {
    ...
    for( ; it_chart != dir->end(); ++it_chart ) {
      ...
      EDA_chart_factory* chart_factory = dynamic_cast<EDA_chart_factory*>(ni.raw_ptr());
      if(chart_factory ==0) continue;

      QString chart_name = chart_factory->title_name();
      QString tab_name = chart_factory->tab_name();
      EDA_chart* chart = chart_factory->get_interface(mdi_area_);
      QTabWidget* tab = 0;
...
EDA_dialog::~EDA_dialog()
{

}


QWidget* EDA_dialog::build_continuous_histogram_selector(QWidget* parent){
...
QWidget* EDA_dialog::build_categorical_histogram_selector(QWidget* parent){
...
void EDA_dialog::show_continuous_histogram(){
...
void EDA_dialog::show_categorical_histogram(){
