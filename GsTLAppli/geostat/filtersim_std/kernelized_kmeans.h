...
//Create the vtkTable
  ...
  for(int f=0; f<nb_filter; ++f) {
    ...
    name<<"Filter "<<f;
    data->SetName(name.str().c_str());
    data->SetNumberOfValues(x.size());
    for(int i=0; i<x.size(); ++i) {
      data->SetValue(i, x[i].second[f]  );
  ...
  kmeans_stat->SetDefaultNumberOfClusters( nb_clusters );
  kmeans_stat->Update() ;

  //Get the actual number of cluster that were generated
  ...
  kmeans_clusters.resize(k_centroids);
  ...
  for(unsigned int r = 0; r < kmeans_stat->GetOutput()->GetNumberOfRows(); r++)
  {
    ...
    kmeans_clusters[ v.ToInt() ].push_back(x[r]);
  }
