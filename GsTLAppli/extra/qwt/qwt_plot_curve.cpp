...
//-------------------------
// added by Nicolas Remy, may 4 2003
void QwtPlotCurve::drawHistogram( QPainter *painter,
			      const QwtScaleMap &xMap, const QwtScaleMap &yMap,
			      int from, int to) const {
    QPolygonF polyline(2 * (to - from) + 1);

    int y_ref = yMap.transform(d_data->reference);

    int i,ip;
    for (i = from, ip = 0; i <= to; i++, ip += 2)
    {
	const int x0 = xMap.transform(x(i));
	const int y0 = yMap.transform(y(i));

	if( i+1 <= to ) {
	    const int x1 = xMap.transform(x(i+1));
	    const int y1 = yMap.transform(y(i+1));
