...
//
// Licensed under the Academic Free Licens eversion 1.1 found in file LICENSE
...
typedef multi_array<GsTLCoord,2>             kdtree2_array;
typedef const_multi_array_ref<GsTLCoord,2>   kdtree2_ro_array;  // read only ref


typedef struct {
  GsTLCoord lower, upper;
} interval;
...
struct kdtree2_result {
  ...
  //
public:
  GsTLCoord dis;  // its square Euclidean distance
...
public:
  ...
  GsTLCoord replace_maxpri_elt_return_new_maxpri(kdtree2_result&);
...
public:
  ...
  void n_nearest_brute_force(vector<GsTLCoord>& qv, int nn, kdtree2_result_vector& result);
  // search for n nearest to a given query vector 'qv' usin
  // exhaustive slow search.  For debugging, usually.

  void n_nearest(vector<GsTLCoord>& qv, int nn, kdtree2_result_vector& result);
  ...
  void r_nearest(vector<GsTLCoord>& qv, GsTLCoord r2,kdtree2_result_vector& result); 
  // search for all neighbors in ball of size (square Euclidean distance)
  // r2.   Return number of neighbors in 'result.size()', 

  void r_nearest_around_point(int idxin, int correltime, GsTLCoord r2,
			      kdtree2_result_vector& result);
  // like 'r_nearest', but around existing point, with decorrelation
  // interval. 

  int r_count(vector<GsTLCoord>& qv, GsTLCoord r2);
...
private:
  ...
  // Give access to a property to check if a element is informed or not
...
private:
  ...
  int select_on_coordinate_value(int c, GsTLCoord alpha, int l, int u); 
...
private:
  ...
  GsTLCoord cut_val, cut_val_left, cut_val_right;  //cut value
